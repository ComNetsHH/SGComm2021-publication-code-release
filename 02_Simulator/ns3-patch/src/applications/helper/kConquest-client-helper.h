...
#ifndef KCONQUEST_HELPER_H
#define KCONQUEST_HELPER_H
 ...
 */
class kConquestBBHelper
{
public:
  /**
   * Create kConquestBBHelper which will make life easier for people trying
   ...
   */
  kConquestBBHelper (uint16_t port);
   ...
   */
  Ptr<kConquestBB> GetServer (void);
private:
  ObjectFactory m_factory; //!< Object factory.
  Ptr<kConquestBB> m_server; //!< The last created server application
};

/**
 * \ingroup kConquestLeafHelper
 * \brief Create a client application which sends UDP packets carrying
 *  a 32bit sequence number and a 64 bit time stamp.
 *
 */
class kConquestLeafHelper
{

public:
  /**
   * Create kConquestLeafHelper which will make life easier for people trying
  ...
  kConquestLeafHelper (Address ip, uint16_t port);
  /**
   *  Create UdpClientHelper which will make life easier for people trying
   * to set up simulations with udp-client-server. Use this variant with
  ...
  kConquestLeafHelper (Address addr);
...
class kConquestTraceClientHelper
{
public:
  /**
   * Create kConquestTraceClientHelper which will make life easier for people trying
   * to set up simulations with udp-client-server.
   *
   */
  kConquestTraceClientHelper ();
   ...
   */
  kConquestTraceClientHelper (Address ip, uint16_t port, std::string filename);
  /**
   ...
   * \param filename the file from which packet traces will be loaded
   */
  kConquestTraceClientHelper (Address addr, std::string filename);
