...
NS_OBJECT_ENSURE_REGISTERED (TwoDiskPropagationLossModel);

TypeId
TwoDiskPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TwoDiskPropagationLossModel")
    .SetParent<PropagationLossModel> ()
    .SetGroupName ("Propagation")
    .AddConstructor<TwoDiskPropagationLossModel> ()
    .AddAttribute ("MaxComRange",
                   "Maximum Transmission Range (meters)",
                   DoubleValue (250),
                   MakeDoubleAccessor (&TwoDiskPropagationLossModel::m_comRange),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("MaxDectRange",
                   "Maximum Detection Range (meters)",
                   DoubleValue (300),
                   MakeDoubleAccessor (&TwoDiskPropagationLossModel::m_dectRange),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("DectPower",
                   "Reported Power within Detection Range (dBm)",
                   DoubleValue (-83),
                   MakeDoubleAccessor (&TwoDiskPropagationLossModel::m_dectPower),
                   MakeDoubleChecker<double> ())
  ;
  return tid;
}

TwoDiskPropagationLossModel::TwoDiskPropagationLossModel ()
{
}

double
TwoDiskPropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                          Ptr<MobilityModel> a,
                                          Ptr<MobilityModel> b) const
{
  double distance = a->GetDistanceFrom (b);
  if (distance <= m_comRange)
    {
      return txPowerDbm;
    }
  else if (distance <= m_dectRange)
    {
      return m_dectPower;
    }
  else
...
int64_t
TwoDiskPropagationLossModel::DoAssignStreams (int64_t stream)
{
  return 0;
}
