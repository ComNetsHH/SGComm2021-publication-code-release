 ...
 * Contributions: Leonard Fisser <leonard.fisser@tuhh.de> for TwoDiskPropagationLossModel
 */

/*
 * 13.04.2021
 * Ammendments done by Leonard Fisser (leonard.fisser@tuhh.de), no change in copyrights.
 */
 ...
 * \ingroup propagation
 *
 * \brief This propagation lowss model models a two disk radio model. Communication between nodes within the first disk is perfect.
 * Communication between nodes inside the second disk is not possibile but detectable.
 * Communication and detection outside both disks is not possibile. It is based on the RangePropagationLossModel.
*/
class TwoDiskPropagationLossModel : public PropagationLossModel
{
public:
...
private:
   ...
   * Defined and unimplemented to avoid misuse
   */
  TwoDiskPropagationLossModel (const TwoDiskPropagationLossModel&);
  /**
   * \brief Copy constructor
   *
   * Defined and unimplemented to avoid misuse
   * \returns
   */
  TwoDiskPropagationLossModel& operator= (const TwoDiskPropagationLossModel&);
...
private:
  double m_comRange; //!< Maximum Transmission Range (meters)
  double m_dectRange; //!< Maximum Detection Range (meters)
  double m_dectPower; //!< Dect Power Level within Disk dectDisk (dBm)
};
